...
func (e *Exporter) scrapeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.Contains(target, "://") && !e.options.AzureRedises.RedisesExists() {
		target = "redis://" + target
	}
...
func (e *Exporter) connectToRedis() (redis.Conn, error) {
	...
	r, success := e.options.AzureRedises.GetRedisCredentialsByAlias(e.redisAddr)
	if success {
		options = append(options, redis.DialPassword(r.key))
		e.redisAddr = r.url
	}
